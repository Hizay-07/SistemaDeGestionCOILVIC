...
public class Ventana_InicioDeSesionControlador implements Initializable {
    ...
    private void limitarTextFields(){
        ComponentesDeVentanaControlador.limitarTextfield(txfd_Usuario, 45);
    ...
    private boolean validarDatosProfesorExterno(){
        ...
        resultado &= validarAuxiliar(()->usuarioAIngresar.setNombreUsuario(txfd_Usuario.getText().toLowerCase()),lbl_ErrorUsuario);
    ...
    public void iniciarSesion(Usuario logger){
        ...
        if(validarDatosProfesorExterno()){
            Usuario usuarioAIngresar = new Usuario();
            usuarioAIngresar.setNombreUsuario(txfd_Usuario.getText().toLowerCase());
